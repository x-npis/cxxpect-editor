...
pub fn show(ui:&mut egui::Ui, text:&mut String, source:Option<&SourceFile>, diagnostics:&[Diagnostic], jump_to:&mut Option<usize>) -> EditorResult {
    ...
    let output=egui::ScrollArea::both().id_salt("editor-scroll").show(ui,|ui|{
        ui.horizontal_top(|ui|{
            ...
            if let Some(range)=out.cursor_range { let index=range.primary.ccursor.index; let prefix=&text[..byte_at_char(text,index)]; cursor_line=prefix.bytes().filter(|b|*b==b'\n').count()+1; cursor_column=prefix.rsplit('\n').next().unwrap_or("").chars().count()+1; }
...
fn highlight(text:&str,wrap:f32,source_text:&str,diags:&[Diagnostic])->LayoutJob{
    ...
    while byte<text.len(){ let rest=&text[byte..];
        ...
        if diags.iter().any(|d| ranges_overlap(byte,byte+end,d.span.start.min(source_text.len()),d.span.end.max(d.span.start+1).min(source_text.len()))){format.underline=Stroke::new(1.0_f32,Color32::from_rgb(244,71,71));}
