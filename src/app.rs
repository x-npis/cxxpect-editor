...
impl eframe::App for EditorApp {
 ...
 fn update(&mut self,ctx:&egui::Context,_frame:&mut eframe::Frame){
  ...
  egui::TopBottomPanel::top("menu").show(ctx,|ui|{egui::menu::bar(ui,|ui|{ui.menu_button("File",|ui|{for (name,c) in [("New",Command::New),("Open...",Command::Open),("Save",Command::Save),("Save as...",Command::SaveAs),("Quit",Command::Quit)]{if ui.button(name).clicked(){self.dispatch(c,ctx);ui.close_menu();}}});ui.menu_button("Edit",|ui|{for (name,c) in [("Undo",Command::Undo),("Redo",Command::Redo),("Find / Replace",Command::Find)]{if ui.button(name).clicked(){self.dispatch(c,ctx);ui.close_menu();}}});ui.menu_button("View",|ui|{if ui.button("Toggle theme").clicked(){self.dispatch(Command::ToggleTheme,ctx);ui.close_menu();}});ui.separator();ui.label(format!("{}{}",self.document.title(),if self.document.is_dirty(){" *"}else{""}));});ui.separator();ui.horizontal(|ui|{for (icon,tip,c) in [("＋","New",Command::New),("◫","Open",Command::Open),("▣","Save",Command::Save),("↶","Undo",Command::Undo),("↷","Redo",Command::Redo),("⌕","Find / Replace",Command::Find),("▶","Next diagnostic",Command::NextDiagnostic)]{if ui.small_button(icon).on_hover_text(tip).clicked(){self.dispatch(c,ctx);}}});});
